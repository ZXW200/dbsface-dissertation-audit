"""Additional similarity, global-feature, and coarse-alignment audits.

This script runs two sensitivity checks for the supplied image-level split:

1. Image-level performance may vary with cross-split image similarity.
...
def main() -> int:
    ...
    summary = f"""# Similarity, Global Statistics, and Alignment Audit

## Similarity-threshold sensitivity

The frozen baseline was re-evaluated after excluding test samples whose maximum cosine similarity to any training image exceeded progressively lower thresholds. This extends the exact and near-duplicate check to broader similarity neighbourhoods.
...
These analyses report exact duplicate checks, broader similarity-threshold exclusions, global image statistics, and coarse alignment summaries for the supplied split.
"""
    print(summary)
    return 0
